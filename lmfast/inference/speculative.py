...
class SpeculativeDecoder:
    ...
    def _select_draft_model(self, target_model: str) -> str | None:
    ...
    def _load_models(self):
        ...
        self.target = AutoModelForCausalLM.from_pretrained(
            self.target_model_path, torch_dtype=torch.float16, device_map=self.device
        )
        self.target.eval()
        ...
        if self.draft_model_path:
            self.draft = AutoModelForCausalLM.from_pretrained(
                self.draft_model_path, torch_dtype=torch.float16, device_map=self.device
            )
            self.draft.eval()
        else:
    ...
    def generate(
        ...
        while tokens_generated < max_tokens:
            ...
            accepted_tokens, new_token = self._verify_tokens(
                generated_ids, draft_tokens, draft_probs, temperature
            )
            ...
            if new_token is not None:
                generated_ids = torch.cat(
                    [generated_ids, new_token.unsqueeze(0).unsqueeze(0)], dim=1
                )
        ...
        if self.stats["draft_calls"] > 0:
            acceptance_rate = self.stats["accepted_tokens"] / (self.stats["draft_calls"] * k)
        else:
            acceptance_rate = 0
        ...
        for i, draft_token in enumerate(draft_tokens[0]):
            ...
            if torch.rand(1).item() < accept_prob:
                ...
                adjusted_probs = torch.clamp(
                    target_probs
                    - F.softmax(logits[0, pos] / self.config.draft_temperature, dim=-1),
                    min=0,
                )
    ...
    def _generate_standard(self, prompt: str, max_tokens: int, temperature: float) -> str:
    ...
    def benchmark(self, prompts: list[str], max_tokens: int = 100) -> dict[str, float]:
        """
        ...
        return {
            ...
            "speedup": speedup,
            "average_acceptance_rate": sum(spec_tokens)
            / (len(prompts) * self.config.speculation_depth * len(prompts)),
        }


def generate_fast(
    model_path: str, prompt: str, draft_model: str | None = None, max_tokens: int = 256, **kwargs
) -> str:
    """
